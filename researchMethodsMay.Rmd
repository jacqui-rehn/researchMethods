...
#May 19
* Created pie charts to showcase proportion of reads aligning at different quality scores
* Generate fragment length plots

#May 22
* Began summarising damage for Weyrich study in R markdown document
* Included plots for count data and fragment lengths
* Some de-bugging needed. Ensure data frames are fully edited at the beginning so that FileNames are consistent, and all code for re-shaping is present in the R document
* Counted number of reads in each split file to use for normalization of fragment length data
...
* Plotted normalized distributions of fragment lengths for non filtered data.
* Need to repeat for Q30 data
* Try calculating summary statistics from vectors and plot rather than generating histogram from whole vector? Is one way preferable.

#May 23

Aims:
* Plot nucleotide substitution data and identify any substitutions for further exploration. 
* Finish plotting high qual fragment length data (compare)
* De-bug R markdown document and complete
* Write flow-chart for processing data (use when meet with Jimmy to discuss R package)

#May 29

Met with Jimmy to share code/progress. Suggestions for improvements:
* make separate file for genome, NCBI ref location, phylum data etc
* use this text file and a while loop to download refSeq fasta file
* select the genome that is easiest to use (e.g. fewest scaffolds for M. oralis)
* replace gsub commands with more concise regular expression
* rather than remove info from fileName, split fileName into separate columns using underscore as defining point. Can then remove unnecessary variables from data frame
* write functions into separate .R file and call them in the R markdown
* use 'cache' to call functions from previous code chunks

Additional questions to consider
* Currently using MAPQ 30. Is this appropriate?:
    * read-up/summarise how MAPQ calculated
    * what is the difference between MAPQ of 25 and 30
    * does including reads with MAPQ >24 increase sample size enough to abolish issues with skewed fragment length??
* Are there more duplicates for lower quality MAPQ filters?
* Adapter dimers have length of ~60bp. Could removal of this account for dips observed in length distribution?

#May 31

Ran following script on 6 samples from Neanderthal. Modification of previous code which takes information from a tab deliminated text file to download & name fasta files
...
See if read lengths in bam files is representative of average read lengths in fastq file

```{bash eval=FALSE}
zcat 2NoAdapt_ModernL7_lAAGAG_rNONE_R1R2_Collapsed.fastq.gz | awk '{if(NR%4==2) print length($1)}' | sort -n | uniq -c > read_length.txt
```
